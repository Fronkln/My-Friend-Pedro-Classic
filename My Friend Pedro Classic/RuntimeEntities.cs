...
namespace MFPClassic
{
    public static class RuntimeEntities
    {
        ...
        public static GameObject SpawnMedkit(Vector3 position)
        {
            GameObject medkit = GameObject.Instantiate(MFPClassicAssets.medkit);
            medkit.transform.position = position;
            Rigidbody medkitRigid = medkit.GetComponent<Rigidbody>();
            medkitRigid.isKinematic = false;
        ...
        public static SpawnDoorScript CreateEnemyDoor(Vector3 position, int enemyCount, bool visualDoor = false)
        {
            ...
            SpawnDoorScript spawnDoorScript = newDoor.GetComponentInChildren<SpawnDoorScript>();

            newDoor.SetActive(true);

            if (visualDoor)
                spawnDoorScript.enabled = false;

            spawnDoorScript.alwaysOnFlatGround = false;
            spawnDoorScript.dontWallCheckLeft = false;
            spawnDoorScript.dontWallCheckRight = false;
