...
public class PlayerScript : MonoBehaviour
{
    ...
    public float zPushBackPublic { get { return zPushBack; } set { zPushBack = value; } }
    ...
    public virtual void Start()
    {
        ...
        GameObject prefabKnife = Instantiate(MFPClassic.MFPClassicAssets.classicBundle.LoadAsset("knife_handle") as GameObject);
        prefabKnife.transform.parent = handR.transform;
        prefabKnife.transform.localPosition = new Vector3(-0.086f, -0.02f, 0.069f);
        prefabKnife.transform.localScale = new Vector3(0.5186859f, 0.5186859f, 0.5186859f);

        Quaternion rot = prefabKnife.transform.localRotation;

        rot.eulerAngles = new Vector3(-21.045f, -248.294f, 149.357f);

        prefabKnife.transform.localRotation = rot;

        this.knife = prefabKnife.transform;
    ...
    public virtual void Update()
    {
        ...
        {
            float num1 = this.root.Damp(zPushBack, this.transform.position.z, 0.1f);
            Vector3 position = this.transform.position;
        ...
        if (this.kFire)
        {
            if (!this.reloading && !this.startedRolling && (!this.root.disableShooting) && (this.automaticWeapon || !this.kFireDown) && ((this.weapon == 5 && this.shotsInARow < 6 || this.weapon != 5) && (double)this.fireDelay <= 0.0))
            {
        ...
        this.fightPoseBlend = (weapon == 0 ? 1 : 0);
    ...
    public virtual void allowKickItem(
      Rigidbody kickRBody,
      float extraGravity,
      float kickRange,
      Vector3 centerOffset,
      PhysicsSoundsScript physicsSoundsScript,
      ObjectKickScript objectKickScript)
    {
        if ((double)this.kickTimer > 5.0 || this.startedRolling && this.onGround)
            return;
        Vector3 vector3_1 = this.transform.InverseTransformPoint(kickRBody.transform.position);
        this.transform.InverseTransformPoint(this.mousePos);
        Vector3 vector3_2 = kickRBody.transform.TransformPoint(centerOffset);
        if ((double)Mathf.Abs(vector3_1.x) >= (double)kickRange || Physics.Linecast(this.transform.position, vector3_2, (int)this.layerMask))
            return;
        this.root.highlightObject(kickRBody.transform, true, this.root.GetTranslation("interact1"), 1.3f);
        this.root.showHintKick = true;
        Vector3 normalized1 = (this.mousePos - vector3_2).normalized;
        Vector3 vector3_3 = 20f * normalized1;
        if ((UnityEngine.Object)this.outlineEffect.outlineRenderers[0] != (UnityEngine.Object)null)
        ...
        for (int length = allEnemies.Length; index < length; ++index)
        {
            Transform transform2 = allEnemies[index].Find("EnemyGraphics/Armature/Center/LowerBack/UpperBack/Neck/Head");
            float num2 = Vector2.Distance((Vector2)vector3_2, (Vector2)transform2.position);
            if ((double)num2 < 8.0 && (double)Vector2.Distance((Vector2)(vector3_2 + normalized1 * num2), (Vector2)transform2.position) < 4.0)
            {
                EnemyScript component = (EnemyScript)allEnemies[index].GetComponent(typeof(EnemyScript));
                if (component.enabled && !component.doorSpawn && ((double)num2 < (double)num1 && !Physics.Linecast(vector3_2, transform2.position, (int)this.layerMask)))
                {
                    num1 = num2;
                    transform1 = transform2;
                    enemyScript = component;
                }
            }
        }
        if ((UnityEngine.Object)transform1 != (UnityEngine.Object)null)
        {
            Vector2 normalized2 = (Vector2)(transform1.position - vector3_2).normalized;
            this.trajectory.rotation = Quaternion.Euler((float)(-(double)Mathf.Atan2(normalized2.y, normalized2.x) * 57.2957801818848) + 90f, 90f, -90f);
            int num2 = 0;
            Vector3 localScale1 = this.trajBone[1].localScale;
            double num3 = (double)(localScale1.x = (float)num2);
            Vector3 vector3_4 = this.trajBone[1].localScale = localScale1;
            float num4 = Vector2.Distance((Vector2)transform1.position, (Vector2)vector3_2);
            Vector3 localScale2 = this.trajBone[0].localScale;
            double num5 = (double)(localScale2.x = num4);
            Vector3 vector3_5 = this.trajBone[0].localScale = localScale2;
            vector3_3 = (Vector3)(normalized2 * 20f);
        }
        else
        {
            this.trajectory.rotation = Quaternion.Euler((float)(-(double)Mathf.Atan2(vector3_3.y, vector3_3.x) * 57.2957801818848) + 90f, 90f, -90f);
            bool flag = new bool();
            Vector2 vector2 = (Vector2)vector3_3;
            int num2 = 1;
            Vector3 localScale1 = this.trajBone[0].localScale;
            double num3 = (double)(localScale1.x = (float)num2);
            Vector3 vector3_4 = this.trajBone[0].localScale = localScale1;
            for (float num4 = 0.0f; (double)num4 < (double)Extensions.get_length((System.Array)this.trajBone); ++num4)
            {
                if ((double)num4 > 0.0)
                {
                    this.trajBone[(int)num4].rotation = Quaternion.LookRotation((Vector3)vector2, Vector3.forward) * Quaternion.Euler(90f, 0.0f, -90f);
                    vector2.y -= (float)(0.5 - (double)extraGravity * (double)Physics.gravity.normalized.y / 8.0 * 0.800000011920929 + (double)kickRBody.drag * ((double)num4 / (double)Extensions.get_length((System.Array)this.trajBone)) * 3.0);
                }
                if (!flag)
                {
                    int num5 = 1;
                    Vector3 localScale2 = this.trajBone[(int)num4].localScale;
                    double num6 = (double)(localScale2.x = (float)num5);
                    Vector3 vector3_5 = this.trajBone[(int)num4].localScale = localScale2;
                    if ((double)num4 >= 7.0)
                    {
                        float num7 = 1f - (float)(((double)num4 - 7.0) / 9.5 * (1.0 - (double)Mathf.Clamp01(Mathf.Abs(normalized1.x) * 1.6f)));
                        Vector3 localScale3 = this.trajBone[(int)num4].localScale;
                        double num8 = (double)(localScale3.x = num7);
                        Vector3 vector3_6 = this.trajBone[(int)num4].localScale = localScale3;
                    }
                    RaycastHit hitInfo = new RaycastHit();
                    if (Physics.Raycast(this.trajBone[(int)num4].position, (Vector3)vector2, out hitInfo, this.trajBone[(int)num4].localScale.x, (int)this.layerMaskIncEnemiesAndEnemyGameCollision))
                    {
                        flag = true;
                        float distance = hitInfo.distance;
                        Vector3 localScale3 = this.trajBone[(int)num4].localScale;
                        double num7 = (double)(localScale3.x = distance);
                        Vector3 vector3_6 = this.trajBone[(int)num4].localScale = localScale3;
                    }
                }
                else
        ...
        if (!this.kPunch && (double)this.punchTimer <= 10.0 || (double)this.itemKickCoolDown > 0.0 || !((UnityEngine.Object)this.root.prevOutlinedObject == (UnityEngine.Object)kickRBody.transform) && !this.root.showHintPressButton)
            return;
        kickRBody.isKinematic = false;
        kickRBody.velocity = vector3_3;
        kickRBody.angularVelocity = kickRBody.velocity;
        if ((UnityEngine.Object)transform1 != (UnityEngine.Object)null)
        {
            ObjectKickScript component = (ObjectKickScript)kickRBody.GetComponent(typeof(ObjectKickScript));
            component.autoTargetTransform = transform1;
            component.autoTargetEnemyScript = enemyScript;
        }
        this.itemKickCoolDown = 20f;
        ...
        if (!((UnityEngine.Object)physicsSoundsScript != (UnityEngine.Object)null))
            return;
        physicsSoundsScript.triggerCollisionSound(3f, true, 0.3f);
    }
